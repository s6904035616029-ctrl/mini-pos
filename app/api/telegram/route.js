...
export default function SellPage() {
  ...
  // 2. ฟังก์ชันยิงข้อความผ่าน Internal API Route (/api/telegram)
  ...
  // 5. คำนวณราคารวมของทั้งตะกร้า
  const grandTotal = cart.reduce((sum, item) => sum + item.total_price, 0);

  // 6. ชำระเงิน / บันทึกการขาย / ตัดสต็อก / ยิงแจ้งเตือน Telegram
  const handleCheckout = async () => {
    ...
    try {
      ...
      // ตัดสต็อกและยิงแจ้งเตือน Telegram
      for (const item of cart) {
        ...
        if (targetProduct) {
          ...
          // แจ้งเตือน 2: สต๊อกเหลือน้อย (<= 5 ชิ้น)
          if (newStock <= 5) {
            const lowStockMsg =
